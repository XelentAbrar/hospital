<template>
  ...
  <AppLayout title="Daily Attendance Report">
    <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-8">
        <div class="bg-blue-50 border border-gray-200 rounded-lg p-8 md:p-12 mb-8 shadow-md">
            <h1 class="text-gray-900 text-3xl md:text-4xl font-extrabold mb-2">Daily Attendance Report</h1>
            <form @submit.prevent="form.get(route('daily-attendance-report.index'))" enctype="multipart/form-data">
...
const form = useForm({
  date: new URLSearchParams(window.location.search).get("date") || new Date().toISOString().substr(0, 10),
});
