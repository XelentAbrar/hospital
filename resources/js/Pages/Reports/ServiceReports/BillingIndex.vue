<template>
  <Head title="Service Billing Report" />
  <AppLayout title="Service Billing Report">
    <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-8">
        <div class="bg-blue-50 border border-gray-300 rounded-lg p-8 md:p-12 mb-8 shadow-md">
            <h1 class="text-gray-900 text-3xl md:text-4xl font-extrabold mb-2">Service billing report</h1>
            <form @submit.prevent="form.get(route('service-billing-reports.index'))" enctype="multipart/form-data">
                <div class="px-4 py-6 sm:p-8">
                    <label for="date" class="block text-lg font-semibold leading-6 text-gray-900">Select Date</label>
                    <div class="mt-2">
                        <flat-pickr
                            class="block cursor-pointer w-full rounded-md border-0 mt-2 py-2.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6"
                            v-model="form.date"
                            :config="dateConfig"
                            >
                        </flat-pickr>
                ...
                <table class="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody class="divide-y divide-gray-300 bg-white">
                    ...
                    >
                      ...
                      >
                        <div class="font-normal text-gray-900 text-sm md:text-base line-clamp-2">
                          <!-- {{ new Date(service.created_at).toISOString().replace('T', ' ').split('.')[0] }} -->
                          {{ new Date(service?.created_at).toLocaleDateString('en-GB').replace(/\//g, '-') }}
...
const form = useForm({
  date: new URLSearchParams(window.location.search).get("type") || null,
});
